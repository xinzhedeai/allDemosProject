...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    data() {
      return {
        favorite: (() => {
          console.info(loadFromLocal(this.seller.id));
    ...
    watch: { // 观察组件中数据的变化
      'seller'() { // 监控seller的值，当异步获取的数据加载进来之后，证明dom的高度也已经被撑开了
        this._initScroll();
        this._initPics();
      }
    },
...
<style scoped lang="stylus" rel="stylesheet/stylus">
  ...
  .seller
    ...
    .overview
      ...
      .favorite
        position absolute
        width 50px // 设定宽度，保证图标奇幻不同状态时位置一致
        right 18px
        top 18px
        // text-align center
        .icon-favorite
